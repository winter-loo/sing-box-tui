...
# Separate operational workspaces from the node dashboard
...
Enter the node dashboard only through an explicit user action, such as the Command Palette action. Inactivity never changes the active page, so dialogs and authentication flows do not need to pause or resume a dashboard navigation timer.

The dashboard is a directly operable destination. Its shortcuts execute immediately, unbound keys do not navigate, and closing its dialogs returns to the dashboard. The navigation menu opens operational pages; arbitrary input does not restore the previous page.
